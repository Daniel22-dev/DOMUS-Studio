...
const TITLE_VERSION = APP_VERSION.replace(/\.0$/, '');
console.log(`http-app-smoke-v73: START (DOMUS ${APP_VERSION})`);
...
for (let attempt = 0; attempt < 120; attempt += 1) {
  ...
  if (loaded.ready && loaded.hasBody && /DOMUS Studio/.test(loaded.title) && loaded.testLab && (loaded.projects >= 1 || loaded.storageError)) break;
  await sleep(150);
}
assert.match(loaded?.title || '', new RegExp(`v?${TITLE_VERSION.replaceAll('.', '\\.')}`), `Titulek aplikace musí odpovídat verzi ${APP_VERSION}.`);
