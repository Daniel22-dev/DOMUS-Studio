...
Write-Host ''
Write-Host 'DOMUS Studio v7.0 - nastaveni volitelne obrazove AI' -ForegroundColor Cyan
...
$model = Read-Host 'Model podporujici obrazovy vstup [gpt-5-mini]'
if ([string]::IsNullOrWhiteSpace($model)) { $model = 'gpt-5-mini' }
@{ model = $model.Trim() } | ConvertTo-Json | Set-Content (Join-Path $configDir 'ai-settings.json') -Encoding UTF8
Write-Host ''
